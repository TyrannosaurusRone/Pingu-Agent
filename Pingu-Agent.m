...
for episode = 1:5000
    ...
    cum_exp_reward = 0;
tic
while ((step < 1000)  && (currentState < 25))
    ...
    [rows, cols] = size(avState); % cols = number of possible actions
    ...
    if rd >= epsilon % Greedy policy finds maximum reward from the possible moves
        ...
        actionV = Qreward(maxIndex,1); % select large
        action = [];
        if sum(actionV ~= 0) > 1
    ...
    for i = 1:cols2 % loops for number of possible actions
         % take the reward value from the Q matrix by indexing with current
         % state and i of possible action
